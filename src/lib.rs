...
impl State {
    ...
    const fn handle_mouse_moved(&mut self, position: PhysicalPosition<f64>) {
    ...
    fn update(&mut self) {
        self.color = Color {
            r: self.cursor_position.x / f64::from(self.config.width),
            g: self.cursor_position.y / f64::from(self.config.height),
            b: 0.0,
            a: 1.0,
        }
    }
